...
🚀 [ReMeFb 快速开始](#使用-reme-package)

💻 [ReMeCli 快速开始](#使用-remecli)
...
[FbSummarizer](reme/memory/file_based/fb_summarizer.py) 是记忆总结的核心组件，它采用 **ReAct+文件工具** 模式
...
当对话过长时，[FbCompactor](reme/memory/file_based/fb_compactor.py) 负责将历史对话压缩为精华摘要——就像写**会议纪要**，把冗长的讨论浓缩成关键要点。
...
async def main():
    ...
    reme = ReMeFb(
        ...
        default_file_store_config={
            ...
            "vector_enabled": True,  # 是否启用向量搜索（无 embedding 服务可设为 False）
        },
...
[ReMe](reme/reme.py) 是基于向量库的记忆系统核心类，支持三种记忆类型的统一管理：

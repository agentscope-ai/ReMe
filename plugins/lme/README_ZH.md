...
插件包含 LongMemEval 的记忆、回答、评分 Step、提示词和完整应用配置。
数据集处理、runner 和结果仍留在 [`benchmark/longmemeval`](../../benchmark/longmemeval/README_ZH.md)。
...
`reme start plugins='["lme"]'` 仅为默认应用注册实现，不增加评测 Job，也不改变默认定时任务。

`config=lme` 和兼容名称 `config=lme.yaml` 加载插件内的
[`configs/lme.yaml`](src/reme_lme/configs/lme.yaml)，其中显式启用插件。

...
from reme_lme.config import config_path
from reme_lme.steps.auto_memory import _interpolate_timestamps
...
def test_backends_are_opt_in_and_do_not_change_default():
    manifest = load_package_manifest("reme_lme", plugin_name="lme")
    ...
    config = resolve_app_config(log_config=False, plugins=["lme"])
...
@pytest.mark.parametrize("name", ["lme", "lme.yaml"])
@pytest.mark.usefixtures("installed_plugin")
...
def test_preset_requires_installed_plugin(monkeypatch):
    ...
    with pytest.raises(ValueError, match="Plugin 'lme' is not installed"):
...
async def test_existing_jobs_start_and_answer_with_mock_models(tmp_path):
    config = resolve_app_config(config="lme.yaml", log_config=False)
    ...
    async with app:
        ...
        judge_agent = _Agent("Yes, the answer matches.")
        ...
        assert result.metadata["raw_answer_judgement"] == "Yes, the answer matches."

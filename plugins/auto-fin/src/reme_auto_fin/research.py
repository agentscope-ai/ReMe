...
class AutoFinResearchStep(AutoFinStep):
    ...
    async def _research(self, topic: str, related: list[dict], run_date: str) -> AutoFinNote:
        ...
        title, path, _ = await self._write_report(
            normalize_title(output.title, f"{topic}观察"),
            output.description,
            output.body,
            kind="auto-fin-topic",
            existing=earlier,
        ...
        return AutoFinNote(topic=topic, title=title, description=output.description, body=output.body, path=path)

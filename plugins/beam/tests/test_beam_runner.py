...
@pytest.mark.parametrize("selection", ["beam", "beam.yaml", "custom", "default"])
...
assert "reme_beam" not in sys.modules
spec = importlib.util.spec_from_file_location("runner", repo / "benchmark/beam/run.py")
...
if selection == "custom":
    ...
    path.write_text(yaml.safe_dump({
        "extends": str(repo / "plugins/beam/src/reme_beam/configs/beam.yaml"),
...
if selection == "default":
    ...
    assert Path(inspect.getfile(cls)).is_relative_to(repo / "plugins/beam/src")
    assert cls().get_prompt("system_prompt")
    assert first.config.plugins == ["beam"]

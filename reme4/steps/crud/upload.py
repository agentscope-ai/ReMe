"""``upload`` — copy an externally-received asset into ``resource/<date>/``.

This step is the **passive** ingest entry point: when an external
channel (wechat group, email, browser save, API push, ...) hands
the agent a file, ``upload`` lands it in ``resource/<YYYY-MM-DD>/``
...
Bucket layout::
    ...
    resource/<YYYY-MM-DD>/
      meta.json                                 # JSON array of ResourceEntry rows
...
6. Appends a ``ResourceEntry`` to ``meta.json`` and re-renders
   ``<date>.md``.
...
* ``description`` (required) — analysis hint for downstream agents:
  ...
  it should be interpreted. The digester / synchronizer reads this
...
from ...utils.resource_utils import assemble_day_md
...
class UploadStep(BaseStep):
    ...
    async def execute(self):
        ...
        set_answer(self.context, outcome)
    ...
    def _land(self, src: Path, date: str, final_name: str, entry_fields: dict) -> dict:
        ...
        with _bucket_lock(lock_path):
            ...
            existing_names = {e.name for e in existing_entries} | on_disk
            if final_name in existing_names:
                raise _DuplicateUpload(
                    f"duplicate: {final_name!r} already exists in {resource_dir}/{date}/",
                )

            shutil.copyfile(src, bucket / final_name)

            entry = ResourceEntry(name=final_name, **entry_fields)
            updated = existing_entries + [entry]
            _atomic_write_text(
                meta_path,
                json.dumps([e.model_dump() for e in updated], ensure_ascii=False, indent=2) + "\n",
            )
            _atomic_write_text(day_md, assemble_day_md(updated, date, resource_dir))

        return {
            "date": date,
            "name": final_name,
            "path": f"{resource_dir}/{date}/{final_name}",
        }
...
def _sanitize_metadata(raw: dict) -> tuple[dict, str]:
    ...
    callers can attach arbitrary tags that land on the
    ``ResourceEntry`` row.
    """
...
def _read_meta(meta_path: Path) -> list[ResourceEntry]:
    """Read ``meta.json`` as a list of entries; missing or malformed → []."""

...
def get_multi_turn_messages() -> list[Message]:
    ...
    return [
        ...
        Message(
            role=Role.USER,
            content="Hello! I'm working on a science project about renewable energy. Can you help me understand the basics?",
        ),
        Message(
            role=Role.ASSISTANT,
            content="Of course! I'd be happy to help. Renewable energy comes from sources that naturally replenish, like solar, wind, hydro, geothermal, and biomass. What specific aspect would you like to explore?",
        ),
        Message(
            role=Role.USER,
            content="I'm particularly interested in solar energy. Can you explain how solar panels work and calculate how much energy a typical home solar system might produce?",
        ),
        Message(
            role=Role.ASSISTANT,
            content="Solar panels work through photovoltaic cells that convert sunlight into electricity. When photons hit the silicon cells, they knock electrons loose, creating an electric current.\n\nFor energy calculation: A typical home solar system is 5-10kW. With average 4-5 peak sun hours per day, a 6kW system would produce approximately 24-30 kWh daily, or 720-900 kWh monthly.",
        ),
        Message(
            role=Role.USER,
            content="That's helpful! Now, given that calculation, if electricity costs $0.12 per kWh, estimate the annual savings. Also, briefly mention what factors might affect this.",
        ),
    ]
...
def get_tool_test_messages() -> list[Message]:
    ...
    return [
        Message(
            role=Role.SYSTEM,
            content="You are a helpful assistant with access to weather and energy calculation tools. Use them when appropriate.",
        ),
        Message(
            role=Role.USER,
            content="I'm planning to install solar panels in San Francisco. Can you help me understand the weather patterns there?",
        ),
        Message(
            role=Role.ASSISTANT,
            content="I'd be happy to help! San Francisco has a Mediterranean climate with mild temperatures year-round. Let me get the current weather data for you.",
        ),
        Message(
            role=Role.USER,
            content="Great! Also, I'm considering monocrystalline panels. If my system produces 800 kWh monthly and electricity costs $0.15 per kWh, what would be my annual savings?",
        ),
    ]
...
Examples:
  ...
  python test_llm_sync.py --all         # Test both LLMs
    ...
    if args.all:
        ...
        # Default to OpenAILLMSync if no argument provided
        llms_to_test = [(OpenAILLMSync, "OpenAILLMSync")]
        print("No LLM specified, defaulting to OpenAILLMSync")
        print("Use --all to test all LLMs, or --openai/--litellm to test a specific one\n")

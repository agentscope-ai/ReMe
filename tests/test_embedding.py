...
async def test_async_single_embedding(model_class: Type[BaseEmbeddingModel], model_name: str):
    ...
    embedding = await model.get_embeddings(test_text)
...
async def test_async_single_node_embedding(model_class: Type[BaseEmbeddingModel], model_name: str):
    ...
    result_node = await model.get_node_embeddings(node)
...
Examples:
  ...
  python test_embedding.py --all         # Test all embedding models
    ...
    if args.all or args.openai:
        ...
        # Default to OpenAIEmbeddingModel if no argument provided
        models_to_test = [(OpenAIEmbeddingModel, "OpenAIEmbeddingModel")]
        print("No model specified, defaulting to OpenAIEmbeddingModel")
        print("Use --all to test all models, or --openai to test OpenAI specifically\n")

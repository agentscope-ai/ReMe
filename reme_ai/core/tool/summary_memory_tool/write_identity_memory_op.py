"""Write identity memory operation for saving agent self-cognition memories.

This module provides the WriteIdentityMemoryOp class for writing identity memories
...
@C.register_op()
class WriteIdentityMemoryOp(BaseMemoryToolOp):
    """Operation for writing identity memories using file-based storage."""
    ...
    def build_input_schema(self) -> dict:
        """Build input schema for identity memory write.

        Returns:
            dict: Input schema for writing an identity memory.
        """
        return {
            "identity_memory": {
                "type": "string",
    ...
    async def async_execute(self):
        """Execute the write identity memory operation.

        Writes identity memory to file storage.
        """
        ...
        if not identity_memory:
            self.set_output("No valid identity memory provided for write.")
            return

        self._save_identity_memory(identity_memory, workspace_id)
        self.set_output(f"Successfully wrote identity memory in workspace={workspace_id}.")

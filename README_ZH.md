...
ReMe 是一个**模块化的记忆管理工具包**，为 AI 智能体提供统一的记忆能力，同时提供基于文件和基于向量的记忆系统。
...
上下文窗口有限，对话太长就丢信息；新对话更是一张白纸，之前的偏好、决策、进度全没了。

ReMe 通过**上下文压缩**和**长期记忆**解决这个问题——旧内容自动浓缩成摘要，重要信息落盘保存，下次对话自动检索。

## 基于文件的记忆设计

记忆就是 Markdown 文件，不依赖外部数据库，随时可看可改：

...
ReMe 的架构设计以认知科学理论为锚点，从第一性原理推导出记忆系统的核心骨架。

- **四层管线：CLS 与事件建构的工程收敛**。互补学习系统理论（CLS）要求快速绑定与缓慢整合分属独立表征空间——`daily/` 充当海马体式的快速绑定层，`digest/` 承载新皮层式的长期语义化；事件建构与多通道感觉加工理论则在输入端增设 `resource/` 与 `session/`，最终收敛为 `resource/ → session/ → daily/ → digest/` 加工级联。`daily/` 同时为适应性遗忘提供未固化的候选集，使遗忘成为主动选择而非容量驱逐。
- **Wikilink：扩散激活理论的仿生联结中枢**。ReMe 以 `[[wikilink]]` 替代传统外键，将四层记忆组织为显式、可遍历的语义网络；`auto_index` 实时调节链接权重，模拟突触可塑性（LTP/LTD）。图谱与 embedding 组合可实现部分线索恢复完整模式，`metadata/` 充当海马体式索引。
- **上层功能围绕基座与 Wikilink 自然生长**。Auto Dream 通过 Wikilink 关联度有选择地整合 daily 至 digest（巩固即选择、未巩固即遗忘），对应突触稳态假说；渐进式混合检索融合 Wikilink 遍历（联想扩展）、BM25（熟悉感通路）与 embedding（情境回想通路），暗合 ACT-R 记忆激活方程。
- **延展心智：人机共享的认知假体**。Markdown 作为认知假体，使人可直接编辑记忆——编辑本身即是生成性再编码。ReMe 将记忆视为人机共建的知识花园：四层管线是土壤，Wikilink 是根系，Auto Dream 是修剪节奏，可读性与可编辑性本身就是认知质量的核心。

...
├── bridge_reme.py            # ReMe ↔ test server 桥接（记忆注入/保存、profile 注入）
...
├── fix_trace_logs.py         # run 输出 → ~/.nanobot/trace_logs 转换（eval 前置）
├── .gitignore                # 排除 env.sh 与全部运行产物
└── config/
    ├── models/reme.yaml      # runner 模型配置（model_id=reme）
    └── bench/evaluation/trace_history.yaml   # trace 采集策略（runner 要求 CWD 相对路径）
```

运行时自动生成（均被 .gitignore 排除）：
`data`（符号链接）、`logs/`、`outputs/`、`reme_workspace/`、`nanobot_workspace/`。
...
- **完成判定**（resume.py）：扫描 `outputs/reme/{persona}/**/history/*-log.jsonl`
  中的 `Task finished task_id=X status=Y`；`SUCCESS/MAX_TURNS/TIMEOUT` 记为完成，
  `ERROR`/未开始的任务重跑（按 episode 顺序以 `--task-id` 传给 runner）。
- **防答案泄漏**：被中断的任务可能已在优雅退出时提炼成 daily 笔记，直接重跑会
  ...
  `session_id = pibench_{task}_*` 匹配），已完成任务的记忆一律不动。
...
- **trace_history.yaml 找不到**：runner 要求 CWD 相对的
  `config/bench/evaluation/trace_history.yaml`；务必从本套件目录运行
  run_persona.sh / run_all.sh（脚本内部已 `cd` 到套件目录）。
